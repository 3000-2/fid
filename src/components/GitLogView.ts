...
export class GitLogView extends BoxRenderable {
  ...
  private getStatusText(): string {
    ...
    return `[Esc] Close  [j/k] Navigate  [g/G] First/Last  [Enter] View Diff  ${current}/${total}`
  }
  ...
  private renderCommits(): void {
    ...
    const maxGraphWidth = Math.max(...this.commits.map((c) => c.graphChars.length))
    ...
    for (let index = 0; index < this.commits.length; index++) {
      ...
      this.rowRenderables.push({ row, message })
    }
